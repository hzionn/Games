...
def load_image(name,sizex=-1,sizey=-1,colorkey=None,): #OK
	#-------------------------------------------------------------------------
	# 圖片調用(開始及結束畫面)
...
def load_sprite_sheet(sheetname,nx,ny,scalex = -1,scaley = -1,colorkey = None,): #OK
	#-------------------------------------------------------------------------
	# 	目的：
	...
	for i in range(0,ny):               
		for j in range(0,nx):
			...
			image.blit(sheet,(0,0),rect)						# 若傳入一個Rect物件，則會使用Rect物件的左上方座標，將image物件繪製在sheet上，達到圖片切割的效果				
...
def extractDigits(number): #OK
	...
	if number > -1:
		...
		for i in range(len(digits),5):	# 若分數不到五位數字，則其他補零
			digits.append(0)
...
class Dino(): # 小恐龍
	def __init__(self,sizex=-1,sizey=-1):	# OK
		...
		self.images,self.rect = load_sprite_sheet('dino.png',5,1,sizex,sizey,-1)  		# 獲得切割後的圖檔以及一個rect物件
		self.images1,self.rect1 = load_sprite_sheet('dino_ducking.png',2,1,59,sizey,-1) # 獲得切割後的圖檔以及一個rect物件
	...
	def checkbounds(self):	# OK
		if self.rect.bottom > int(0.98*height):	# 確保小龍的位置一直都站在0.98*height的位置上
	...
	def update(self):	# OK
		...
		if not self.isDead and self.counter % 7 == 6 and self.isBlinking == False:	# 計算分數
			self.score += 1
...
class Cactus(pygame.sprite.Sprite):	# 仙人掌
	def __init__(self,speed=5,sizex=-1,sizey=-1):	# OK
		...
		self.images,self.rect = load_sprite_sheet('cacti-small.png',3,1,sizex,sizey,-1)	# 獲得切割後的圖檔以及一個rect物件
...
class Ptera(pygame.sprite.Sprite): # 飛鳥
	def __init__(self,speed=5,sizex=-1,sizey=-1): # OK
		pygame.sprite.Sprite.__init__(self,self.containers)
		self.images,self.rect = load_sprite_sheet('ptera.png',2,1,sizex,sizey,-1) 	# 獲得切割後的圖檔以及一個rect物件
...
class Cloud(pygame.sprite.Sprite): # 雲
	def __init__(self,x,y):
		pygame.sprite.Sprite.__init__(self,self.containers)
		self.image,self.rect = load_image('cloud.png',int(90*30/42),30,-1)	# 載入雲的圖片
		self.speed = 1						# 設定移動速度
...
class Scoreboard(): # 記分板
	def __init__(self,x=-1,y=-1): # OK
		self.score = 0
		self.tempimages,self.temprect = load_sprite_sheet('numbers.png',12,1,11,int(11*6/5),-1) # 載入數字
		...
		if pygame.display.get_surface() == None:
			...
			for event in pygame.event.get():
				...
				if event.type == pygame.KEYDOWN:	
					if event.key == pygame.K_SPACE or event.key == pygame.K_UP: # 按下空白建或是上方向鍵
...
def gameplay():
	...
	retbutton_image,retbutton_rect = load_image('replay_button.png',35,31,-1) 	# 載入圖片
	gameover_image,gameover_rect = load_image('game_over.png',190,11,-1) 		# 載入圖片

	#將數字圖片分離成單一
	temp_images,temp_rect = load_sprite_sheet('numbers.png',12,1,11,int(11*6/5),-1)	# 載入數字
	...
	while not gameQuit:
		...
		while not gameOver:
			...
			#-------------------------------------------------------------------------
					if event.type == pygame.KEYDOWN:
						...
						if event.key == pygame.K_SPACE:
							if playerDino.rect.bottom == int(0.98*height): # 保證小恐龍不能在空中跳躍
			...
			for c in cacti:
				...
				if pygame.sprite.collide_mask(playerDino,c): # collide_mask用來檢測兩個物件是否碰撞
			...
			for p in pteras:
				...
				if pygame.sprite.collide_mask(playerDino,p): # collide_mask用來檢測兩個物件是否碰撞
					playerDino.isDead = True 	# 有碰撞，小恐龍死亡 
					if pygame.mixer.get_init() != None:
						die_sound.play()		# 播放死亡的音效
			...
			if len(cacti) < 2: 	# 控制螢幕上仙人掌的數量，最多2個
				if len(cacti) == 0: 							# 沒有仙人掌的情況下
					...
					for l in last_obstacle:
						if l.rect.right < width*0.7 and random.randrange(0,50) == 10: # 還有其他障礙物，但快要消失 (用random控制生成，並不是每次都會生成仙人掌)
							last_obstacle.empty()							# 清空 last_obstacle 這個group物件
							last_obstacle.add(Cactus(gamespeed, 40, 40))	# 並新增仙人掌

			if len(pteras) == 0 and random.randrange(0,200) == 10 and counter > 500: # 沒有飛鳥且時間大於500才會生成飛鳥 (用random控制生成，並不是每次都會生成飛鳥)
				for l in last_obstacle:
			...
			if len(clouds) < 5 and random.randrange(0,300) == 10:	# 控制螢幕上雲朵的數量，最多5個  (用random控制生成，並不是每次都會生成飛鳥)
