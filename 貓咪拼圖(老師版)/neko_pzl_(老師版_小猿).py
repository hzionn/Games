...
def check_neko():                   #判斷貓咪是否於垂直、水平、傾斜的方向連成一線，，而且數量大於等於3個以上的函數
    ...
    for y in range(1, 9):           
        for x in range(8):
            if check[y][x] > 0:
                if check[y-1][x] == check[y][x] and check[y+1][x] == check[y][x]:  #當格子裡有貓咪，上下又是相同貓咪，就讓這些貓咪都變肉球
                    neko[y-1][x] = 7
                    neko[y][x] = 7
                    neko[y+1][x] = 7
    ...
    for y in range(10):
        for x in range(1, 7):
            if check[y][x] > 0:
                if check[y][x-1] == check[y][x] and check[y][x+1] == check[y][x]:  #當格子裡有貓咪，左右又是相同貓咪，就讓這些貓咪都變肉球
                    neko[y][x-1] = 7
                    neko[y][x] = 7
                    neko[y][x+1] = 7
    ...
    for y in range(1, 9):
        for x in range(1, 7):
            if check[y][x] > 0:
                if check[y-1][x-1] == check[y][x] and check[y+1][x+1] == check[y][x]:  #當格子裡有貓咪，左上 右下 又是相同貓咪，就讓這些貓咪都變肉球
                    neko[y-1][x-1] = 7
                    neko[y][x] = 7
                    neko[y+1][x+1] = 7
                if check[y+1][x-1] == check[y][x] and check[y-1][x+1] == check[y][x]:  #當格子裡有貓咪，左下 右上 又是相同貓咪，就讓這些貓咪都變肉球
                    neko[y+1][x-1] = 7
                    neko[y][x] = 7
                    neko[y-1][x+1] = 7
...
def drop_neko():                    #由下往上檢查，若有值且下方為空，則將值往下放置
    ...
    for y in range(8, -1, -1):      #迴圈 y 從8遞減至0
        for x in range(8):          #迴圈 y 從0遞增至7
...
def game_main():
    ...
    if index == 0: # 標題的標誌   #任務三：改成小猿小猿
        draw_txt("小猿小猿", int(312*scale), int(240*scale), int(100*scale), "violet", "TITLE")
...
root = tkinter.Tk()
root.title("掉落物拼圖「小猿小猿」")                    #任務三：改成小猿小猿
...
bg = tkinter.PhotoImage(file="ape_bg.png")            #任務三：圖片改成ape_bg.png
cursor = tkinter.PhotoImage(file="neko_cursor.png")
img_neko = [                                          #利用列表管理多張圖片，將img_neko[0]設定為空白值
    None,
    tkinter.PhotoImage(file="ape1.png"),              #任務三：圖片改成ape1.png
